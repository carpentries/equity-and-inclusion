## 2018 Demographics for Software Carpentry Learners
...
opts_chunk$set(echo = FALSE,
               message = FALSE,
               warning = FALSE)
opts_chunk$set(fig.path='figures/2018_learners_') #puts all figures in figures folder
```


```{r include=FALSE}
# Loading dataset for the 2018 pre-workshop survey demographics for Software Carpentry
swc_pre_2018 <- readr::read_csv("https://raw.githubusercontent.com/carpentries/equity-and-inclusion/master/demographics/software-carpentry/2018/2018-swc-pre-demographics.csv")
```
...
swc_pre_2018$`First-Workshop` = factor(swc_pre_2018$`First-Workshop`, levels = first_time)
...
swc_pre_2018$`Position` = factor(swc_pre_2018$`Position`, levels = position)
...
ggplot(position_tally,
  ...
  ylab("% Respondents") +
  ggtitle("2018 Breakdown of Respondents by Position") +
...
n_responses_to_domain_question <-
  n_responses_to_the_question(swc_pre_2018,
                              from_colname = Domain,
                              to_colname = Column61)
...
swc_pre_2018$`Gender` = factor(swc_pre_2018$`Gender`, levels = gender)
...
scale_x_discrete(labels = function(x) lapply(strwrap(x,
    ...
    xlab("Gender") +
    ylab("% respondents") +
    ggtitle("2018 Breakdown of Respondents by Gender") +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme_classic(base_size = 14)
```
...
swc_pre_2018$`Race-Ethnicity` = factor(swc_pre_2018$`Race-Ethnicity`, levels = race)
...
scale_x_discrete(labels = function(x) lapply(strwrap(x,
    ...
    xlab("Race/Ethnicity") +
    ylab("% respondents") +
    ggtitle("2018 Breakdown of U.S. Respondents by Race/Ethnicity") +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme_classic(base_size = 14)
