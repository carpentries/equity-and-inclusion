## 2017 Demographics for Software Carpentry Learners
...
```{r include=FALSE}
# Loading dataset for the 2017 pre-workshop survey demographics for Software Carpentry
swc_pre_2017 <- readr::read_csv("https://raw.githubusercontent.com/carpentries/equity-and-inclusion/master/demographics/software-carpentry/2017/2017-swc-pre-demographics.csv")
```
...
The following includes demographic data for those individuals who took Software Carpentry's [pre-workshop survey](https://github.com/carpentries/assessment/blob/master/learner-assessment/surveys/swc_presurvey.pdf) in 2017. There were a total 5707 learners who completed this survey in 2017.
...
swc_pre_2017$`First-Workshop` = factor(swc_pre_2017$`First-Workshop`, levels = first_time)
...
swc_pre_2017$`Position` = factor(swc_pre_2017$`Position`, levels = position)
...
n_responses_to_domain_question <-
  n_responses_to_the_question(swc_pre_2017,
                              from_colname = Domain,
                              to_colname = Column61)
...
swc_pre_2017$`Gender` = factor(swc_pre_2017$`Gender`, levels = gender)
...
scale_x_discrete(labels = function(x) lapply(strwrap(x,
    ...
    xlab("Gender") +
    ylab("% respondents") +
    ggtitle("2017 Breakdown of Respondents by Gender") +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme_classic(base_size = 14)
```
...
swc_pre_2017$`Race-Ethnicity` = factor(swc_pre_2017$`Race-Ethnicity`, levels = race)
...
scale_x_discrete(labels = function(x) lapply(strwrap(x,
    ...
    xlab("Race/Ethnicity") +
    ylab("% respondents") +
    ggtitle("2017 Breakdown of U.S. Respondents by Race/Ethnicity") +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme_classic(base_size = 14)
